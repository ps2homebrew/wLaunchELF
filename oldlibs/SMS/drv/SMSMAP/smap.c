...
#define SMAP_PP_CLK_OUT(pSMap, C)                                                           \
    {                                                                                       \
        (pSMap)->u8PPWC = (C) ? ((pSMap)->u8PPWC | PP_SCLK) : ((pSMap)->u8PPWC & ~PP_SCLK); \
...
static u16 ComputeFreeSize(SMapCB const *pCB)
{
    ...
    return u16Start > u16End ? (SMAP_TXBUFSIZE + u16End - u16Start) : (SMAP_TXBUFSIZE - u16End + u16Start);
} /* end ComputeFreeSize */
