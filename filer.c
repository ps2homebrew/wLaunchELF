	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			drawSprite(setting->color[0],
				...
				SCREEN_WIDTH, y/2+10);
			if (swapKeys)
				printXY("~:OK ›:Cancel ¢:Back", x, y/2, setting->color[2], TRUE);
			else
				printXY("›:OK ~:Cancel ¢:Back", x, y/2, setting->color[2], TRUE);
		}//ends if(event||post_event)
		drawScr();
	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			drawSprite(setting->color[0],
				...
				SCREEN_WIDTH, y/2+10);
			if (swapKeys)
				printXY("~:Use ›:Clear  :Set ¢:Back R1/L1:Page_left/right", x, y/2, setting->color[2], TRUE);
			else
				printXY("›:Use ~:Clear  :Set ¢:Back R1/L1:Page_left/right", x, y/2, setting->color[2], TRUE);
		}//ends if(event||post_event)
		drawScr();
	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			if(cnfmode==TRUE){
				...
				if(!strcmp(ext, "*")) {
					if (swapKeys)
						sprintf(msg1, "~:OK ›:Cancel ¢:Up  :*->JPG R2:PathPad");
					else
						sprintf(msg1, "›:OK ~:Cancel ¢:Up  :*->JPG R2:PathPad");
				} else {
					if (swapKeys)
				...
				if(title_show) {
					if (swapKeys) 
						sprintf(msg1, "~:OK ¢:Up ›:Mark  :RevMark L1/L2:TitleOFF R1:Menu R2:PathPad Sel:Exit");
					else
						sprintf(msg1, "›:OK ¢:Up ~:Mark  :RevMark L1/L2:TitleOFF R1:Menu R2:PathPad Sel:Exit");
				} else {
					if (swapKeys) 
						sprintf(msg1, "~:OK ¢:Up ›:Mark  :RevMark L1/L2:TitleON  R1:Menu R2:PathPad Sel:Exit");
					else
						sprintf(msg1, "›:OK ¢:Up ~:Mark  :RevMark L1/L2:TitleON  R1:Menu R2:PathPad Sel:Exit");
				}
			}
