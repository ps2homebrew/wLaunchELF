	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			drawSprite(setting->color[0],
				...
				SCREEN_WIDTH, y+16);
			if (swapKeys)
				sprintf(c, "я1:%s я0:%s я3:%s я</я::-/+128%s L1/R1:-/+1%s L2/R2:-/+10%s",
					LNG(OK), LNG(Cancel), LNG(Back), LNG(MB), LNG(GB), LNG(GB));
			else
				sprintf(c, "я0:%s я1:%s я3:%s я</я::-/+128%s L1/R1:-/+1%s L2/R2:-/+10%s",
					LNG(OK), LNG(Cancel), LNG(Back), LNG(MB), LNG(GB), LNG(GB));
	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			drawSprite(setting->color[0],
				...
				SCREEN_WIDTH, y+16);
			if (swapKeys)
				sprintf(tmp, "я1:%s я0:%s я3:%s", LNG(OK), LNG(Cancel), LNG(Back));
			else
				sprintf(tmp, "я0:%s я1:%s я3:%s", LNG(OK), LNG(Cancel), LNG(Back));
...
void hddManager(void)
{
	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event
			...
			//Tooltip section
			sprintf(tooltip, "R1:%s  я3:%s", LNG(MENU), LNG(Exit));
			if(PartyInfo[browser_sel].Treatment == TREAT_HDL_RAW){
				sprintf(tmp, " я2:%s", LNG(Load_HDL_Game_Info));
				strcat(tooltip, tmp);
			}
